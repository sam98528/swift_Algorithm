        ...
        with open(readme_path, "r", encoding="utf-8") as f:
            ...
            for index, line in enumerate(readme_lines):
                if "제출 일자" in line:
                    ...
                    link = line[link_index:-2]  # 문제 링크를 추출
...
def main():
    ...
    content = "| 알고리즘 사이트 | 난이도 | 문제번호 | 풀이 | 제출일자 |\n" + "| ------------- | ------------- | ------------- | ------------- | ------------- |\n"
    content_entries = []
    sorting_entries = []
    directory_count = 0
        ...
        for file in files:
            if file == 'README.md':
                if category not in solveds:
                    ...
                    if submission_date:
                        entry = "| <center> [{}]({} </center>|<center> {} </center>|<center> {} </center>|<center>[Link]({}) </center>|<center> {} </center>|\n".format("문제 링크",link, "LV " + directory, category, parse.quote(os.path.join(root)), submission_date.strftime("%Y-%m-%d"))
                    else:
